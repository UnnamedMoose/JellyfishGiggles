...
# ===
plot(xlabel="x/L", ylabel="y/L", aspect_ratio=:equal, size=(1000, 600))

# ---
xy, cps, segParams1, backbone = shapeForTime(0., kinematics_0_baseline)
...
if plotPython
    plot!(shape_Costello_regressed[:, 1] .+ 0.0, shape_Costello_regressed[:, 2], linewidth=2, color=:black, label="")
    plot!(refCps[:, 1] .+ 0.0, refCps[:, 2], linewidth=2, marker=:x, linestyle=:dashdotdot, color=:black,
        markersize=5, label="")
end

# ---
xy, cps, segParams2, backbone = shapeForTime(0.13, kinematics_0_baseline)
...
if plotPython
    ...
    plot!(shape_Costello_regressed[:, 1] .+ 0.5, shape_Costello_regressed[:, 2], linewidth=2, color=:black, label="")
    plot!(refCps[:, 1] .+ 0.5, refCps[:, 2], linewidth=2, marker=:x, linestyle=:dashdotdot, color=:black,
        markersize=5, label="")
end

# ---
xy, cps, segParams3, backbone = shapeForTime(0.27, kinematics_0_baseline)
...
if plotPython
    ...
    plot!(shape_Costello_regressed[:, 1] .+ 1.0, shape_Costello_regressed[:, 2], linewidth=2, color=:black, label="")
...
function generate_frames()
    anim = Animation()
    for x in 0:0.01:1
        xy, cps, segParams, backbone = shapeForTime(x, kinematics_0_baseline)
        fs = 14
        ...
        shape_Costello_regressed = readdlm("./dataset_01_medusae/shape_Costello2020_snapshot2.txt")
        plot!(shape_Costello_regressed[:, 1], shape_Costello_regressed[:, 2], linewidth=2, color=:green,
            linestyle=:dash, label="Costello et al., t/T=0.13")

        shape_Costello_regressed = readdlm("./dataset_01_medusae/shape_Costello2020_snapshot3.txt")
        plot!(shape_Costello_regressed[:, 1], shape_Costello_regressed[:, 2], linewidth=2, color=:blue,
...
function generate_frames()
    anim = Animation()
#    for x in [0.1, 0.11, 0.12, 0.13, 0.14, 0.15]
    for x in 0:0.01:1
        fs = 14
        plot(xlabel="x/L", ylabel="y/L", aspect_ratio=:equal, xlims=(0, 0.75), ylims=(-0.75, 0), size=(1000, 800),
            xtickfontsize=fs, ytickfontsize=fs, xguidefontsize=fs, yguidefontsize=fs, legendfontsize=fs)
        ...
        xy, cps, segParams, backbone = shapeForTime(x, kinematics_0_baseline)
        plot!(xy[1, :], xy[2, :], color=:blue, linewidth=3, label="Baseline")
        plot!(cps[1, :], cps[2, :], linewidth=2, color=:blue, marker=:circle, linestyle=:dash, markersize=4, alpha=0.5, label="")

        xy, cps, segParams, backbone = shapeForTime(x, kinematics_1_noFlick)
        plot!(xy[1, :], xy[2, :], color=:orange, linewidth=3, label="No flick")
        plot!(cps[1, :], cps[2, :], linewidth=2, color=:orange, marker=:circle, linestyle=:dash, markersize=4, alpha=0.5, label="")
        
        xy, cps, segParams, backbone = shapeForTime(x, kinematics_2_largeFlick)
        plot!(xy[1, :], xy[2, :], color=:green, linewidth=3, label="Large flick")
        plot!(cps[1, :], cps[2, :], linewidth=2, color=:green, marker=:circle, linestyle=:dash, markersize=4, alpha=0.5, label="")

        shape_Costello_regressed = readdlm("./dataset_01_medusae/shape_Costello2020_snapshot1.txt")
        plot!(shape_Costello_regressed[:, 1], shape_Costello_regressed[:, 2], linewidth=2, color=:black, linestyle=:dash, label="")
        ...
        #savefig("outputs/plot_02_frame_bellShape_differentKinematics_$formatted_float.png")
...
# Save the animation as a GIF
gif(anim, "outputs/plot_03_animatedBellShapes_differentKinematics.gif", fps=10)
