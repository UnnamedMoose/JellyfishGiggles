using StaticArrays, WaterLily, CUDA, ParametricBodies, Interpolations, LinearAlgebra, Dierckx, BiotSavartBCs, CSV, Statistics
...
function generate_jelly_motion(phase_contr, phase_exp, Ncps, T1, T2, Tg, n_cycles, n_up, γ; ThreeD=false, varyingT=false, gliding=false)
    ...
    elseif varyingT == true && gliding == false
        period_frames_T2    = interpolate_cycle_duty(cps_seq; n_contr=4, n_exp=6, base_up=Int((T2/T1)*n_up), γ=γ)
    ...
    elseif varyingT == true && gliding == true
        time_set    = range(0,(T1+Tg+T2+Tg) * n_cycles,length=length(sx_smooth[1]))   
    end
...
function jelly_simulation_3D(duration, D, period, Re, U, ϵ, pathing, Uff, Ncps)
    ...
    sim         =   BiotSimulation((3D, D, D), (Uff,Uff,Uff), D; U, ν=(U*D) / Re, body, T, mem=CuArray, ϵ, nonbiotfaces=(-2,-3))
...
pathing  = generate_jelly_motion(phase_contr, phase_exp, Ncps, T1, T2, Tg, n_cycles, n_up, γ; ThreeD=true, varyingT=false, gliding=true)

jelly_simulation_3D(duration, D, (T1+Tg), Re, U, ϵ, pathing, Uff, Ncps)
