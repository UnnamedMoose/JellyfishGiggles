using StaticArrays, WaterLily, ParametricBodies, Interpolations, LinearAlgebra, Dierckx, BiotSavartBCs, CSV, Statistics

import WaterLily: @loop,scale_u!,conv_diff!,udf!,accelerate!,BDIM!

import WaterLily: CFL
CFL(a::Flow;Δt_max=10) = 0.25
...
function interpolate_cycle_duty(cps_seq; n_contr::Int, n_exp::Int, base_up::Int=10, γ::Real=1)
    ...
    γ_corr = γ / (4/10) 
    up_e = base_up
    up_c = round(Int,γ_corr * base_up)

    n_c_samples = (n_contr-1)*up_c + 1
    n_e_samples = (n_exp)*up_e + 1

    t_c = range(1, n_contr, length = n_c_samples)[1:end]
    t_e = range(n_contr, Nframes, length = n_e_samples)[2:end]
...
function run_jelly_simulation(duration,period, D, pathing, Ncps, deg; Re=302, U=1, Uff=0, ϵ=1)
    cps         =   cps_at_time(pathing, Ncps, 0;) .* D .+ SA{T}[D/2, 0] 
    ...
    sim         =   BiotSimulation((3D, D), (Uff,Uff), D; U, ν=(U*D)/Re, body, mem=Array,ϵ, nonbiotfaces=(-2))

    t₀ = round(sim_time(sim)); step = 0.1; t0 = 0; a0 = 0; v0 = 0; p0 = 0; f0 = 0
    ...
    open("Data/Simulation_Data/results_2D.csv", "w") do io
        ...
        for tᵢ in range(t₀, t₀ + (duration * period); step)        
            ...
            while t < tᵢ * sim.L / sim.U
                cps             = cps_at_time(pathing, Ncps, t * sim.U/sim.L) .* D .+ SA{T}[D/2, 0] 
...
pathing         = generate_jelly_motion(phase_contr, phase_exp, Ncps, T1, T2, Tg, n_cycles, n_up, γ; ThreeD=false, varyingT=false, gliding=false)

WaterLily.logger("Data/Simulation_Data/test_psolver")
run_jelly_simulation(duration,T1, D, pathing, Ncps, deg; Re=Re, U=U, Uff=Uff, ϵ=ϵ)
