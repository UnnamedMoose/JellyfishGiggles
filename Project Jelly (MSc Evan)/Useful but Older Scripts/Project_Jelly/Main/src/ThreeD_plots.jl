...
function makie_video!(sim, dat, obs_update!, makie_plot;
                      remeasure=false, name="file.mp4", duration=1,
                      step=0.1, framerate=30, compression=20)

    fig = Figure(resolution=(800, 800))
    ax = Axis3(fig[1, 1])
    
    obs = obs_update!(dat, sim) |> Observable
    volume!(ax, obs)  # Always show something basic
    makie_plot(obs, ax)  # Now pass both obs and axis to let user add plots
    
    t₀ = round(sim_time(sim))
    t = range(t₀, t₀ + duration; step)
    
    Makie.record(fig, name, t; framerate, compression) do tᵢ
        sim_step!(sim, tᵢ; remeasure)
        obs[] = obs_update!(dat, sim)
        println("simulation ", round(Int, (tᵢ - t₀) / duration * 100), "% complete")
    end
